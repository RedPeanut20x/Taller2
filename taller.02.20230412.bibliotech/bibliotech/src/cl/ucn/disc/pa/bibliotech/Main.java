...
public final class Main {
    ...
    private static void cambiarContrasenia(Sistema sistema) throws IOException {
        while (true)
        {
            ...
            if(sistema.obtenerSocioLogeado().getContrasenia().equals(nuevaContrasenia))
            {
                StdOut.println("Debe de ingresar una contraseña diferente a la actual");
            }
            else
            {
                sistema.obtenerSocioLogeado().setContrasenia(nuevaContrasenia);
                sistema.guardarInformacion();
                StdOut.println("");
                StdOut.println("¡Su contraseña ha sido cambiada correctamente!");
                StdOut.println("");
    ...
    private static void editarCorreo(Sistema sistema)
    {
        while(true)
        {
            StdOut.println("[*] Editar email [*]");
            StdOut.println("""
                    [#] Ingrese su nuevo email : """);
            String nuevoCorreo = StdIn.readLine();

            if(sistema.obtenerSocioLogeado().getCorreoElectronico().equals(nuevoCorreo))
            {
                StdOut.println("Debe de ingresar una contraseña diferente a la actual");
            }
            else
